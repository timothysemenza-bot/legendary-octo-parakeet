...
test('apply homepage content in Kajabi editor (write-gated)', async ({ page, context }) => {
  ...
  const heroHeadline = env('JWBLNG_HOMEPAGE_HEADLINE', 'A Professional Home for Jewish Business Leaders').trim();
  ...
  const primaryCtaLink = env('JWBLNG_HOMEPAGE_PRIMARY_CTA_LINK', '/join').trim();
